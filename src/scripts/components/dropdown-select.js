(() => {
  ...
  window.customElements.define('dropdown-select', class DropdownSelect extends HTMLElement { // eslint-disable-line
      ...
      shiftOption(event, move) {
        event.preventDefault();
        const validOptions = [...this.select.options].filter(
          (opt, index) =>
            !opt.disabled &&
            ((move === -1 && index < this.select.selectedIndex) || (move === 1 && index > this.select.selectedIndex)),
        );
      ...
      optionValueChange(option) {
        ...
        if (siteBody.classList.contains('template-collection')) {
          this.select.querySelector(`option[value=${this.select.value}]`).innerHTML = `${
            theme.strings.sortBy
          } ${this.select
            .querySelector(`option[value=${this.select.value}]`)
            .innerHTML.replace(theme.strings.sortBy, '')}`;
        }
      ...
      searchForStringOption(string) {
        const foundOptions = this.options.filter(
          (opt, index) => opt.innerText.toLowerCase().indexOf(string) === 0 && !opt.hasAttribute('data-disabled'),
        );
