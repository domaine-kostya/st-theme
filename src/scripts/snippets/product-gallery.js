...
newCustomElement(
  ...
  class extends HTMLElement {
    constructor() {
      super();
      this.swiperEl = this.querySelector('[data-product-gallery-carousel]');
    ...
    buildPaginationSwiperSlides() {
      return this.filteredSlides.map(
        (slide, index) =>
          `<div class="swiper-slide swiper-pagination-thumbnail swiper-pagination-thumbnail--slide">${slide.dataset.thumbImage}</div>`,
      );
    }
    ...
    initSwiper() {
      ...
      const swiperOptions = {
        ...
        initialSlide: 0,
        navigation: {
          nextEl: this.querySelector('[data-product-gallery-nav="next"]'),
          prevEl: this.querySelector('[data-product-gallery-nav="prev"]'),
        },
      };
    ...
    set selectedVariant(variantJson) {
      ...
      if (!this.swiper) {
        this.initCarouselGallery();
        this.activeColor = variantColor;
        return;
      }

      if (this.isFilterItemsByColor === 'true') {
        if (variantColor === this.activeColor) {
          return;
        }
        ...
        const matchingMediaItem = [...this.mediaEls].find((mediaItem) =>
          mediaItem.dataset.variantIds?.includes(variantJson.id),
        );
...
newCustomElement(
  ...
  class extends HTMLElement {
    ...
    set selectedVariant(variantJson) {
      const variantColor = variantJson[`option${this.colorOptionIndex}`];
      const activeMediaItem = [...this.mediaItems].find((mediaItem) =>
        mediaItem.dataset.variantIds?.includes(variantJson.id),
      );
      ...
      const activeThumbnail = [...this.thumbnails].find(
        (thumbnail) => thumbnail.dataset.mediaId === activeMediaItem.dataset.mediaId,
      );
